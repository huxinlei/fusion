 ...
 */
class JimProcessor implements Processor {

    /**
     * 日志打印
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(JimProcessor.class);
    ...
    public void tick(Tick tick, Session session) {
        ...
        if (session.getStatus() == Session.STATUS.ESTABLISHED) {
            Packet packet = new Packet((short) 1, Account.P_ACCOUNT_LOGIN_REQ.newBuilder().setToken(UUID.randomUUID().toString()).setName("Jim").build().toByteArray());
