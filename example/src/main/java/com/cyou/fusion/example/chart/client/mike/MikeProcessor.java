 ...
 * MikeProcessor
 * <p>
 * Created by zhanglei_js on 2017/5/16.
 */
class MikeProcessor implements Processor {

    /**
     * 日志打印
     */
    private static final Logger LOGGER = LoggerFactory.getLogger(MikeProcessor.class);
    ...
    public void tick(Tick tick, Session session) {
        ...
        if (session.getStatus() == Session.STATUS.ESTABLISHED) {
            Packet packet = new Packet((short) 1, Account.P_ACCOUNT_LOGIN_REQ.newBuilder().setToken(UUID.randomUUID().toString()).setName("Mike").build().toByteArray());
